...
void Element::display(QTreeWidgetItem *me, PaintInfo *paintInfo, const bool setItem)
{
    ...
    case ET_ELEMENT:
        ...
        if(!areChildrenLeavesHidden(me)) {
            me->setIcon(0, isBookmarked ? elementBkmIcon : elementIcon);
        } else {
            me->setIcon(0, isBookmarked ? childrenHiddenBkmIcon : childrenHiddenIcon);
        }
        {
            QString qualifiedInfo = styleElementTag(me, paintInfo);
            ...
            while(tt.hasNext()) {
                ...
                if(tx->isCDATA) {
                    ...
                    if(isCompactView) {
                        textToShow.append(limitLargeTextWithEllipsis(tx->text.trimmed()));
                    } else {
        ...
        break;
    case ET_PROCESSING_INSTRUCTION: {
        firstColText += tr("*Processing Instruction* ?").append(getPITarget()) ;
        me->setText(0, firstColText);
        me->setForeground(0, procInstrBrush);
        me->setIcon(0, isBookmarked ? procInstrBkmIcon : procInstrIcon);
    ...
    case ET_COMMENT: {
        ...
        me->setIcon(0, isBookmarked ? commentBkmIcon : commentIcon);
        QString textToShow ;
        if(isCompactView) {
    ...
    case ET_TEXT: {
        ...
        me->setText(0, firstColText);
        me->setForeground(0, commentBrush);
        me->setIcon(0, isBookmarked ? textBkmIcon : textIcon);
        QString textToShow ;
        if(isCompactView) {
            textToShow = limitTextWithEllipsis(text);
            attrTxt = textToShow;
    ...
    default:
        ...
        me->setIcon(0, isBookmarked ? bookmarkIcon : QIcon());
        break;
    }
    ...
    if(!attributes.isEmpty()) {
        ...
        setAttributesText(me, paintInfo, attrList);
        if(skipped) {
            me->setIcon(paintInfo->columnForAttributes, filteredAttributesIcon);
        } else {
            me->setIcon(paintInfo->columnForAttributes, QIcon());
        }
